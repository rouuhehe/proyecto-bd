...
def generar_personas_y_roles(n=20):
    personas = []
    for _ in range(n):
...
def asignar_roles(personas):
    ...
    propietarios = personas[:len(personas)//2]
    empleados = personas[len(personas)//2:]
...
def crear_citas_y_diagnosticos(veterinarios, recepcionistas, mascotas):
    ...
    for _ in range(15):
        ...
        cur.execute("INSERT INTO Diagnostico (id_cita, fecha, sintomas, observaciones) VALUES (%s, %s, %s, %s)", (id_cita, fecha, sintomas, observaciones))
    return citas
...
if __name__ == "__main__":
    personas = generar_personas_y_roles(30)
